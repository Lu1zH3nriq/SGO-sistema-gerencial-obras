import React, { useEffect } from 'react'; import { Routes, Route, Navigate, useNavigate } from 'react-router-dom';

import LoginPage from '../../pages/Login/Login.js'; import Dashboard from '../../pages/Dashboard/Dashboard.js'; import Obras from '../../pages/Obras/Obras.js'; import Unauthorized from '../../pages/Unauthorized/Unauthorized.js';

import { useUIContextController, setUserType, setUserLogin, setUserToken, setUserName, setUserId } from '../../context/index.js';

const Authenticator = () => {
    const navigate = useNavigate(); const [controller, dispatch] = useUIContextController(); const { userLogin } = controller;
    ...
    const handleLogin = (user) => {
        if (!user || !user.userType || !user.userToken || !user.userName || !user.userId) {
